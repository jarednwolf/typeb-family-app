...
/**
 * HTTP endpoint to receive performance metrics from the app
 */
...
/**
 * Batch endpoint for receiving multiple performance metrics
 */
...
export const aggregatePerformanceMetrics = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      // Clean up old raw metrics (keep last 24 hours)
...
export const generateHourlyPerformanceReport = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      // Group by metric name and calculate hourly averages
...
export const trackNavigationPerformance = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    // Convert to performance metrics for aggregation
    const metrics: PerformanceMetric[] = [
      {
        name: `nav_transition_${navMetric.screen}`,
        value: navMetric.transitionDuration,
        unit: 'ms',
        timestamp: navMetric.timestamp,
        platform: navMetric.platform,
        appVersion: '',
        metadata: {
          screen: navMetric.screen,
          previousScreen: navMetric.previousScreen,
        },
      },
      {
        name: `nav_render_${navMetric.screen}`,
        value: navMetric.renderDuration,
        unit: 'ms',
        timestamp: navMetric.timestamp,
        platform: navMetric.platform,
        appVersion: '',
      ...
      {
        name: `nav_interaction_${navMetric.screen}`,
        value: navMetric.interactionReady,
        unit: 'ms',
        timestamp: navMetric.timestamp,
        platform: navMetric.platform,
        appVersion: '',
    ...
    // Save as performance metrics too
    const batch = db.batch();
    metrics.forEach(metric => {
      const metricRef = db.collection('performanceMetrics').doc();
...
async function checkPerformanceDegradation(report: any) {
  // Get previous hour's report for comparison
  ...
  for (const currentMetric of report.metrics) {
    ...
    // Check for significant degradation (>20% increase in p90)
...
async function cleanupOldMetrics() {
  ...
  // Clean up old navigation metrics
