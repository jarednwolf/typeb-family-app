/**
 * Family Service Integration Tests with Firebase Emulator (Simplified)
 * Tests the refactored family service with real Firebase emulator
 * Uses simplified cleanup to avoid security rule issues
 */
...
describe('Family Service Integration Tests (Simplified)', () => {
  ...
  describe('joinFamily', () => {
    it('should join family successfully', async () => {
      ...
      const family = await createFamily(db, parentUser.uid, 'Join Test Family');
    ...
    it('should handle case-insensitive invite codes', async () => {
      // Create a family
      const family = await createFamily(db, parentUser.uid, 'Case Test Family');
      const inviteCode = family.inviteCode;
    ...
    it('should respect family capacity limits', async () => {
      // Create a family
      const family = await createFamily(db, parentUser.uid, 'Capacity Test Family');
      const inviteCode = family.inviteCode;
    ...
    it('should prevent joining multiple families', async () => {
      // Create first family
      const family1 = await createFamily(db, parentUser.uid, 'First Family');
      
      // Create second family
      const family2 = await createFamily(db, secondParentUser.uid, 'Second Family');

      // Try to join first family when already in second
      await expect(joinFamily(db, secondParentUser.uid, family1.inviteCode))
        .rejects.toThrow('You are already in a family');
    });
  });
  ...
  describe('Family Member Management', () => {
    it('should remove family member', async () => {
      ...
      const family = await createFamily(db, parentUser.uid, 'Member Test Family');
    ...
    it('should change member role', async () => {
      // Create family with members
      const family = await createFamily(db, parentUser.uid, 'Role Test Family');
    ...
    it('should prevent removing self', async () => {
      // Create family
      const family = await createFamily(db, parentUser.uid, 'Self Remove Test Family');
    ...
    it('should prevent demoting last parent', async () => {
      // Create family
      const family = await createFamily(db, parentUser.uid, 'Demote Test Family');
    ...
    it('should require parent permission for admin actions', async () => {
      // Create family
      const family = await createFamily(db, parentUser.uid, 'Permission Test Family');
  ...
  describe('leaveFamily', () => {
    it('should allow member to leave family', async () => {
      // Create family with members
      const family = await createFamily(db, parentUser.uid, 'Leave Test Family');
      ...
      // Sign in as parent to verify family was updated
      await signInWithEmailAndPassword(auth, parentUser.email!, 'password123');
      const updatedFamily = await getFamily(db, parentUser.uid, family.id);
      expect(updatedFamily?.memberIds).not.toContain(childUser.uid);
    });
    ...
    it('should prevent last parent from leaving with members', async () => {
      // Create family with members
      const family = await createFamily(db, parentUser.uid, 'Last Parent Test Family');
    ...
    it('should allow last member to leave', async () => {
      // Create family with members
      const family = await createFamily(db, parentUser.uid, 'Last Member Test Family');
