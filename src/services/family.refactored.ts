/**
 * Family management service (Refactored for better testability)
 * Handles all family-related operations with Firestore
 * 
 * REFACTORING CHANGES:
 * - Removed dependency on global auth.currentUser
 * - All functions now accept currentUserId as parameter
 * - Firestore instance is passed as parameter for better testability
 ...
 * @param isPremium - Whether this is a premium family
 */
  ...
  try {
    return await runTransaction(db, async (transaction) => {
      ...
      if (userData.familyId) {
        ...
        const existingFamilyDoc = await transaction.get(doc(db, 'families', userData.familyId));
      ...
      const newFamily: Omit<Family, 'id'> = {
        ...
        parentIds: [currentUserId], // Creator is automatically a parent
        childIds: [],
 ...
 * @param db - Firestore instance
 * @param currentUserId - The authenticated user's ID
 * @param inviteCode - The family invite code
 * @param role - The role to assign (default: 'child')
 */
export const joinFamily = async (
  ...
  try {
    return await runTransaction(db, async (transaction) => {
      ...
      if (family.memberIds.includes(currentUserId)) {
      ...
      transaction.update(doc(db, 'families', family.id), {
        memberIds: arrayUnion(currentUserId),
        [roleArray]: arrayUnion(currentUserId),
        updatedAt: serverTimestamp(),
      });
      ...
      return {
        ...family,
        memberIds: [...family.memberIds, currentUserId],
        [roleArray]: [...family[roleArray], currentUserId],
      };
    });
  } catch (error: any) {
 ...
 * @param db - Firestore instance
 * @param currentUserId - The authenticated user's ID
 * @param familyId - The family ID to retrieve
 */
...
export const getFamilyMembers = async (
  ...
  try {
    ...
    const { family } = await validateFamilyPermission(db, currentUserId, familyId, 'view');

    const members: User[] = [];
    
    // Fetch all member documents using Promise.all for better performance
...
export const updateFamily = async (
  ...
  const { family } = await validateFamilyPermission(db, currentUserId, familyId, 'update');
  ...
  if (updates.maxMembers !== undefined) {
    ...
    if (!family.isPremium && updates.maxMembers > 4) {
      throw new Error('Premium subscription required for more than 4 members');
    }
  }
  ...
  try {
    await runTransaction(db, async (transaction) => {
      // Update family
      transaction.update(doc(db, 'families', familyId), {
        ...updates,
        updatedAt: serverTimestamp(),
      });
    });
...
export const removeFamilyMember = async (
  ...
  const { family } = await validateFamilyPermission(db, currentUserId, familyId, 'admin');
...
export const leaveFamily = async (
  ...
  const { family } = await validateFamilyPermission(db, currentUserId, userData.familyId, 'leave');

  // Determine user's role
  const role = family.parentIds.includes(currentUserId) ? 'parent' : 'child';
  const roleArray = role === 'parent' ? 'parentIds' : 'childIds';

  // Handle orphaned tasks BEFORE transaction
  await handleOrphanedTasks(userData.familyId, currentUserId, family.createdBy);

  try {
    await runTransaction(db, async (transaction) => {
      // Remove user from family
      transaction.update(doc(db, 'families', userData.familyId), {
        memberIds: arrayRemove(currentUserId),
        [roleArray]: arrayRemove(currentUserId),
        updatedAt: serverTimestamp(),
      });
 ...
 * @param db - Firestore instance
 * @param currentUserId - The authenticated user's ID
 * @param memberIds - Array of member IDs to monitor
 * @param callback - Callback function for updates
 */
...
export const regenerateInviteCode = async (
  ...
  // Validate admin permission BEFORE transaction
  await validateFamilyPermission(db, currentUserId, familyId, 'admin');
...
export const canPerformAdminAction = async (
  ...
  try {
    const family = await getFamily(db, currentUserId, familyId);
    if (!family) {
      return false;
    }

    // Only parents can perform admin actions
    return family.parentIds.includes(currentUserId);
  } catch (error) {
